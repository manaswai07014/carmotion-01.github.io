...
async def fetch_keyword_rss(session: aiohttp.ClientSession, keyword: str,
    ...
    async with semaphore:
        ...
        try:
            ...
            # Use session's async request instead of urllib
            # Fall back to sync urllib for simplicity in async context
            await asyncio.sleep(0.3)  # rate limit

            # Use sync urllib in executor to not block
            ...
            for item in items:
                ...
                if pub_date:
                    try:
                        # Try RFC 822 format (standard RSS)
                        import email.utils
...
def load_history() -> dict:
    """Load 7-day history from JSONL. Returns {keyword: [daily_counts]."""
...
def main():
    ...
    if pytrends_ok and pytrends_results:
        ...
        try:
            ...
            print(f"  ✅ Source 2: RSS OK — {len(valid)} keywords scored (v3)")
            for r in scored[:5]:
    ...
    print(f"\n📋 Data Freshness: {data_freshness} | Window: {RECENCY_WINDOW_HOURS}h | Velocity: 7-day avg")
